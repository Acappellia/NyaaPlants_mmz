...
execute unless data storage np:tmp mainhand.tag.np_seed run return -1

data remove storage np:tmp plant_info
data modify storage np:tmp plant_info.plantid set from storage np:tmp mainhand.tag.np_seed
